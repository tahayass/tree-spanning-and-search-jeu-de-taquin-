...
def manhatten(array):
    r=array[array!=final_node]
    Total_distance=0
    for num in r:
        index=(np.argwhere(array==num))[0]
        correct_index=(np.argwhere(final_node==num))[0]
        distance_M=abs(index[0]-correct_index[0])+abs(index[1]-correct_index[1])
        Total_distance=distance_M+Total_distance
...
def expand(node_x,open_list):
    ...
    for s in l : 
        ...
        if s=='r':
            ...
            t.array=node_temp
            t.path.append('r')
            t.h=(9-np.count_nonzero(t.array==final_node))*3+manhatten(t.array)
...
def A_3H1M(init_node,time_limit,memory_limit):
    previous_depth=0
    N_nodes_treated=0
