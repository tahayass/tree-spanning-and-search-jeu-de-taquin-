...
def expand(node_x,open_list):
    ...
    for s in l : 
        ...
        if s=='r':
            try:
                if t.path[-1]=='l': #saves from 10-20% memory
                    continue
            except: 
                pass
...
def node_in_closed_list(node,cl):
    i=0
    while i < len(cl):
        if ((node.array==cl[i].array).all()==True):
            return True 
        i=i+1
    return False

def DFS(init_node,depth_limit,time_limit,memory_limit):
    final_node=np.array([[1,2,3],[4,5,6],[7,8,0]])
    previous_depth=0
    N_nodes_treated=0
    nodes_that_repeats=0
    open_list=[]
    closed_list=[init_node]
    ...
    while N_nodes_treated<100000 :
        ...
        if (time.clock() - t0 > time_limit) :
            ...
            returns=[[-1],0,N_nodes_treated,len(open_list),(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,time_limit]
            break
        #### Processing limit    
        if N_nodes_treated==99999 : #stops when procedures takes too much time
            ...
            print('depth reached: ',len(first_node.path))
            print('Memory(open and closed lists) usage: ',(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,' Kbytes')
            returns=[[-1],-1,100000,-1,(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,-1]
            break
        ...
        if (len(open_list)>memory_limit):
            ...
            print('Memory(open and closed lists) usage: ',(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,' Kbytes')
            print('depth reached: ',len(first_node.path))
            returns=[[-1],0,N_nodes_treated,len(open_list),(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,time.clock() - t0]
            break            
            
        if len(open_list)==0 : 
            print('No solution in depth',depth_limit)
            return [[-1],-1,N_nodes_treated,-1,(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,time.clock() - t0]
        first_node=open_list.pop(0)
        
        
        ### Prevent repeating nodes and infinite repeating cycles
        if node_in_closed_list(first_node,closed_list): #check if the node is treated before
        ...
        closed_list.insert(0,first_node) #pour les problèmes de bouclages
        ...
        if len(first_node.path)==depth_limit:
            continue
        else:        
