...
def BFS(init_node,time_limit,memory_limit):
    ...
    while N_nodes_treated<100000 :
        ...
        if (len(open_list)>memory_limit) :
            print('On a dépasser',memory_limit,'noeuds dans la liste, Solution peut être possible mais coûteuse')
            print('Memory(open and closed lists) usage: ',(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,' Kbytes')
            print('depth reached: ',len(first_node.path))
            returns=returns=[['No solution under memory limit'],0,N_nodes_treated,len(open_list),(sys.getsizeof(open_list)+sys.getsizeof(closed_list))/1000,time.clock() - t0]
            break
        ...
        if (first_node.array==final_node).all()==True :
            ...
            print(sys.getsizeof(first_node))
